...
class GocodeProvider implements AutocompleteProvider {
  ...
  getSuggestions (options: SuggestionRequest): Promise<Array<Suggestion>> {
    ...
    if (prefix !== '.' && prefix.length === 1 && nonWordChars.includes(prefix)) {
      // it's a non word char like ()[]{}... so we can assume
      // that the previous suggestions are invalid now
      this.currentSuggestions = []
    }
