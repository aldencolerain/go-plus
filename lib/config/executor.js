...
class Executor {
  ...
  containerDirectory() {
    if (this.cachedContainerDirectory === undefined) {
      const ops = this.getDefaultOptions();
      ops.cwd = projectPath();
      const result = this.execSync("postal", ["exec", "dev", "pwd"], ops);
      this.cachedContainerDirectory = result.stdout.trim();
    }
    return this.cachedContainerDirectory;
  }

  // update command, args, and options to call command inside container using postal
  proxy(command, args, options) {
    // console.log(">>> proxy");
    // console.log(command, args, options);
    const container = atom.config.get('go-plus.config.postal');
    if (container) {
      
      let original = [command].concat(args).join(" ");
      const cwd = options.cwd.replace(projectPath(), this.containerDirectory());
      command = "postal";
      args = ["exec", container, "cd " + cwd + " && " + original];
  ...
  exec (command, args = [], options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const proxied = that.proxy(command, args, options);
